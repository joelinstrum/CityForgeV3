"""Render source-derived directional far views for two colonial houses.

Run with Blender -b --python Tools/build_colonial_houses_far_views.py --
    [output-root]. The untouched archives are extracted to ignored staging.
Both houses use the same neutral camera, 10 m height, and 50 px/m contract.
"""
import math
import sys
import zipfile
...
ARCHIVES = ROOT / 'Documentation/Migration/SourceArchives/ColonialHousesV01'
STAGING = ROOT / 'Authoring/Buildings/ColonialHousesV01/Source'
DEFAULT_OUT = ROOT / 'Assets/CityForgeV3/Resources/CityForgeV3/Buildings3D'
OUTPUT = Path(sys.argv[sys.argv.index('--') + 1]) if '--' in sys.argv else DEFAULT_OUT
HOUSES = (
    ('ColonialHouseA', 'colonial-house.zip'),
    ('ColonialHouseB', 'colonia-house-2.zip'),
)


def build(name, archive_name):
    source_dir = STAGING / name
    source_dir.mkdir(parents=True, exist_ok=True)
    if not list(source_dir.glob('*.fbx')):
        with zipfile.ZipFile(ARCHIVES / archive_name) as archive:
            archive.extractall(source_dir)
    fbx = next(source_dir.glob('*.fbx'))
    out = OUTPUT / name / 'FarViews'
