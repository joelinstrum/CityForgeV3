"""Pack the walking pair's directional renders for AutomataClipPlayer."""
import argparse
from pathlib import Path

from PIL import Image


parser = argparse.ArgumentParser()
parser.add_argument('render_dir', type=Path)
parser.add_argument('output_dir', type=Path)
args = parser.parse_args()
args.output_dir.mkdir(parents=True, exist_ok=True)
clip_id = 'gentleman-and-lady-strolling-v01'
for facing in range(8):
    sheet = Image.new('RGBA', (256 * 8, 160 * 5))
    for frame in range(40):
        path = args.render_dir / f'direction-{facing}-frame-{frame:02d}.png'
        with Image.open(path) as image:
            if image.size != (256, 160):
                raise ValueError(f'Wrong frame size: {path}: {image.size}')
            sheet.paste(image.convert('RGBA'),
                        ((frame % 8) * 256, (frame // 8) * 160))
    out = args.output_dir / f'{clip_id}-facing-{facing}.png'
    sheet.save(out, optimize=True)
    print(out)
with Image.open(args.render_dir / 'direction-0-frame-06.png') as image:
    image.save(args.output_dir / 'thumbnail.png', optimize=True)
