...
[InitializeOnLoad]public static class GrassNeutralReview
{
    static GrassNeutralReview(){EditorApplication.update+=Poll;}
    static void Poll(){const string file="/tmp/cityforge-grass-neutral-command.txt";if(EditorApplication.isCompiling||!File.Exists(file))return;var c=File.ReadAllText(file).Trim();File.Delete(file);try{
        string result="OK "+c;
        if(c=="import"){
            ...
            else if(c=="off")ground.SetFloat("_GrassNeutralStrength",0);
            else if(c=="on")ground.SetFloat("_GrassNeutralStrength",1);
            else if(c=="check"){
                var t=ground.GetTexture("_BrownTex") as Texture2D;
                if(ground.shader.name!="CityForgeV3/MountainGroundSurfaceV05"||ShaderUtil.ShaderHasError(ground.shader)||t==null||t.mipmapCount<2||t.wrapMode!=TextureWrapMode.Mirror)throw new Exception("Grass material validation failed");
                result="PASS shader=true bound=true mipmaps=true mirrored=true texture="+t.name+" size="+t.width+"x"+t.height;
            }
        }
        File.WriteAllText("/tmp/cityforge-grass-neutral-result.txt",result);
    }catch(Exception e){File.WriteAllText("/tmp/cityforge-grass-neutral-result.txt",e.ToString());}}
}
#endif
