    ...
    static void Poll(){const string file="/tmp/cityforge-mountain-transitions-command.txt";if(EditorApplication.isCompiling||!File.Exists(file))return;var c=File.ReadAllText(file).Trim();File.Delete(file);try{
        ...
        if(c=="import"){
            ...
            if(c=="noon")world.SetTimeOfDay(TimeOfDayPreset.Noon);
                ...
                if(ground.shader.name!="CityForgeV3/MountainGroundSurfaceV06"||ShaderUtil.ShaderHasError(ground.shader)||t==null||t.mipmapCount<2||t.wrapMode!=TextureWrapMode.Mirror)throw new Exception("Grass material validation failed");
                result="PASS shader=true bound=true mipmaps=true mirrored=true texture="+t.name+" size="+t.width+"x"+t.height;
            }
        }
        File.WriteAllText("/tmp/cityforge-mountain-transitions-result.txt",result);
    }catch(Exception e){File.WriteAllText("/tmp/cityforge-mountain-transitions-result.txt",e.ToString());}}
}
#endif
