    ...
    static void Poll(){const string file="/tmp/cityforge-forest-green-command.txt";if(EditorApplication.isCompiling||!File.Exists(file))return;var c=File.ReadAllText(file).Trim();File.Delete(file);try{
        ...
        if(c=="import"){
            ...
            if(c=="noon")world.SetTimeOfDay(TimeOfDayPreset.Noon);
                ...
                if(ground.shader.name!="CityForgeV3/MountainGroundSurfaceV07"||ShaderUtil.ShaderHasError(ground.shader)||t==null||t.mipmapCount<2||t.wrapMode!=TextureWrapMode.Mirror)throw new Exception("Grass material validation failed");
        ...
        File.WriteAllText("/tmp/cityforge-forest-green-result.txt",result);
    }catch(Exception e){File.WriteAllText("/tmp/cityforge-forest-green-result.txt",e.ToString());}}
}
#endif
