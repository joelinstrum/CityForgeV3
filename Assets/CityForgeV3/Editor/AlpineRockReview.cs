...
[InitializeOnLoad]public static class AlpineRockReview
{
    static AlpineRockReview(){EditorApplication.update+=Poll;}
    static void Poll(){const string file="/tmp/cityforge-alpine-rock-command.txt";if(EditorApplication.isCompiling||!File.Exists(file))return;var c=File.ReadAllText(file).Trim();File.Delete(file);try{
        string result="OK "+c;
        if(c=="import"){
            ...
            if(c=="noon")world.SetTimeOfDay(TimeOfDayPreset.Noon);
            ...
            else if(c=="off")ground.SetFloat("_TransitionStrength",0);
            else if(c=="on")ground.SetFloat("_TransitionStrength",1);
            else if(c=="check"){
                ...
                if(ground.shader.name!="CityForgeV3/MountainGroundSurfaceV08"||ShaderUtil.ShaderHasError(ground.shader)||t==null||t.mipmapCount<2||t.wrapMode!=TextureWrapMode.Mirror)throw new Exception("Grass material validation failed");
        ...
        File.WriteAllText("/tmp/cityforge-alpine-rock-result.txt",result);
    }catch(Exception e){File.WriteAllText("/tmp/cityforge-alpine-rock-result.txt",e.ToString());}}
}
#endif
