...
[InitializeOnLoad]public static class QuietSilverReview
{
 static QuietSilverReview(){EditorApplication.update+=Poll;}
 static void Poll(){const string file="/tmp/cityforge-quiet-silver-command.txt";if(EditorApplication.isCompiling||!File.Exists(file))return;var c=File.ReadAllText(file).Trim();File.Delete(file);try{
 ...
 if(ShaderUtil.ShaderHasError(ground.shader))throw new Exception("Terrain shader failed");
 }
 File.WriteAllText("/tmp/cityforge-quiet-silver-result.txt","PASS "+c+" quiet silver source, terrain and crag bindings, mips, mirror, shaders");
 }catch(Exception e){File.WriteAllText("/tmp/cityforge-quiet-silver-result.txt",e.ToString());}}
}
#endif
