...
namespace CityForgeV3.World
{
    public static class DistrictIndustryRotation
    {
            ...
            out Vector2 center, out Vector2 half, out float yaw)
        {
            ...
            if (item.Kind == DistrictSelectionKind.Quarry)
            {
                var site = d?.StoneSites?.Find(s => s.Id == item.Id && s.Built);
                if (site == null) return false;
                center = DistrictQuarry.Point(d, site); half = new Vector2(15, 14); yaw = site.Yaw; return true;
            }
            if (item.Kind == DistrictSelectionKind.Brickworks)
            {
                var site = d?.Brickworks?.Find(s => s.Id == item.Id);
                if (site == null) return false;
                center = DistrictBrickworks.Point(d, site); half = new Vector2(10, 12.5f); yaw = site.Yaw; return true;
            }
            return false;
        }
        ...
        public static void Apply(RegionCityTile d, DistrictSelectionRef item, int direction)
        {
            ...
            if (item.Kind == DistrictSelectionKind.Quarry)
            {
                var q = d.StoneSites.Find(s => s.Id == item.Id);
            ...
            {
                var b = d.Brickworks.Find(s => s.Id == item.Id);
