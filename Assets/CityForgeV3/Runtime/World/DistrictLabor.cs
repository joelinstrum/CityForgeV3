...
{
    public enum AxemanActivity { Waiting, Walking, Chopping, WaitingForFall, Delivering, Returning }
    ...
    public static class DistrictLabor
    {
        ...
        {
            var changes=new DistrictLaborChanges();var s=State(d);if(dt<=0 || !s.CampPlaced)return changes;
            s.SeasonSeconds+=dt;
            ...
            foreach(var w in s.Workers.ToArray())
            {
                ...
                {claimed.Remove(w.TreeId);w.TreeId="";w.Route=route(w.Position,s.Camp);w.Activity=AxemanActivity.Returning;changes.Durable=true;}
                if(w.Activity is AxemanActivity.Returning or AxemanActivity.Delivering)
                {
                    if(w.Route==null || w.Route.Count==0 && Vector2.Distance(w.Position,s.Camp)>.1f)w.Route=route(w.Position,s.Camp);
                    if(!Move(w,s.Camp,dt,walkable))continue;
                    int deposit=Math.Min(w.Cargo,int.MaxValue-s.Wood);s.Wood+=deposit;w.Cargo-=deposit;
                    if(w.Cargo>0)continue;
                ...
                if(w.Activity==AxemanActivity.Waiting)
                {
                    ...
                    foreach(var candidate in (d.Flora??new()).Where(t=>t!=null && !claimed.Contains(t.InstanceId) && (DistrictTreeHarvest.CanFell(t) || t.FloraId=="cilician-fir" && t.HarvestState==DistrictTreeHarvestState.Fallen && t.RemainingWood>0)).OrderBy(t=>(TreePoint(d,t)-w.Position).sqrMagnitude))
                    {
                ...
                else if(w.Activity==AxemanActivity.Chopping)
                {
                    ...
                    {
                        if(DistrictTreeHarvest.Fell(tree,0)){changes.Changed.Add(tree.InstanceId);changes.Falling.Add(tree.InstanceId);}
                ...
                else if(w.Activity==AxemanActivity.WaitingForFall)
                {
                    ...
                    w.Cargo+=DistrictTreeHarvest.TakeWood(tree,8);changes.Changed.Add(tree.InstanceId);claimed.Remove(w.TreeId);w.TreeId="";w.Route=route(w.Position,s.Camp);w.Activity=AxemanActivity.Delivering;w.Progress=0;changes.Durable=true;
                }
            }
            return changes;
        }
