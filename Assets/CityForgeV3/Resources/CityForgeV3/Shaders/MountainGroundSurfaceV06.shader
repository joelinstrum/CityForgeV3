Shader "CityForgeV3/MountainGroundSurfaceV06"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed3 NeutralGrass(fixed3 color)
            {
                ...
                float3 candidate = color * float3(.84, 1.0, 1.24);
            ...
            fixed4 frag(VertexToFragment input) : SV_Target
            {
                ...
                fixed3 grass = NeutralGrass(TriplanarRock(_MainTex, p * (12.0 / 5.0), normal));
                ...
                float brownMask = smoothstep(.42,.66,brownPatch) * .85 * _BrownStrength;
                ...
                float3 grassVariation = lerp(float3(.94,1.015,.97),float3(1.045,.975,.92),dry);
                ...
                fixed3 scree = lerp(shale,brown,.32);
