Shader "CityForgeV3/MountainGroundSurfaceV10"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(VertexToFragment input) : SV_Target
            {
                ...
                float edgeBreakup = (RockNoise(p / 3.2 + float3(17,29,5)) - .5) * .12;
                float rockMask = smoothstep(0.46, 0.70, exposure + edgeBreakup) * saturate(_RockEnabled);
                ...
                // Slope changes where vegetation grows rather than making every patch translucent.
                float greenThreshold = lerp(.76, .48, shelf);
                float shelteredGreen = smoothstep(greenThreshold-.06,greenThreshold+.06,patch+edgeBreakup*.5) * .82;
                ...
                debris = smoothstep(.20,.48,debris);
