...
public static class TrapperQa
{
    const BindingFlags F=BindingFlags.Instance|BindingFlags.Public|BindingFlags.NonPublic;
    const string Resource="CityForgeV3/Props/Animals/TrapperV01/Trapper_Walk_Trot_Idle_v01";
    ...
    static void Fixture()
    {
        ...
        var p=world.Session.Data.Props[index];p.PropId=LotWorldController.TrapperPropId;p.AnimationState="idle";p.PositionX=25f;p.PositionZ=-20f;p.RotationQuarterTurns=0;p.MovementX=p.MovementZ=0;
    ...
    [MenuItem("City Forge/QA/Trapper/Open Mounted Rider")]
    static void Open(){if(EditorApplication.isPlaying)Fixture();}
    [MenuItem("City Forge/QA/Trapper/Check Imported Gaits")]
    static void Check()
    {
        Directory.CreateDirectory("QA/Trapper");var model=Object.Instantiate(Resources.Load<GameObject>(Resource));model.SetActive(false);
        ...
        Object.DestroyImmediate(model);File.WriteAllText("QA/Trapper/imported-clips.txt",report);
        if(EditorApplication.isPlaying)Fixture();
    }
    [MenuItem("City Forge/QA/Trapper/Preview Click To Ride")]
    static void Preview()
    {
        ...
        Directory.CreateDirectory("QA/Trapper");File.WriteAllText("QA/Trapper/live.txt",$"destinationAccepted={accepted} selectedName={world.SelectedAnimalName}\n");
    ...
    static void Tick()
    {
        ...
        File.AppendAllText("QA/Trapper/live.txt",$"walked={walked} hooves={hooves.Length} hoofMotion={hoofMotion} riderBobMeters={maxRider-minRider} destinationError={Vector2.Distance(new Vector2(pos.x,pos.z),destination)} finalGait={player.State} travelSpeed={root.GetComponent<HorseGaitController>().TravelSpeed}\n");
