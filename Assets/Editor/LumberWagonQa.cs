...
public static class LumberWagonQa
{
    ...
    [MenuItem("City Forge/QA/Lumber Wagon/Open Track Preview")]
    static void Open()
    {
        if(!EditorApplication.isPlaying)return;
        Fixture(LotWorldController.HorseLumberWagonPropId);Frame();
        Directory.CreateDirectory("QA/LumberWagon");
        ...
        File.WriteAllText("QA/LumberWagon/rig.txt",report);
    }
    [MenuItem("City Forge/QA/Lumber Wagon/Check Shared Driving")]
    static void Check()
    {
        if(!EditorApplication.isPlaying)return;
        Directory.CreateDirectory("QA/LumberWagon");var report="";
        foreach(var id in new[]{LotWorldController.HorseCarriagePropId,LotWorldController.HorseLumberWagonPropId})
        {
            ...
            report+=$"{id}: accepted={accepted} wraps={wraps} firstSpeed={firstSpeed} cruise={cruise} wheels={wheelController.WheelCount} wheelsMoved={wheelsMoved} maxHitchError={hitch} maxSteer={maxSteer} coasts={coasts} brakingAfter1s={braking} rebuildPreservesMotion={persists} parked={parked} stable={point==t.transform.position} shadows={shadows}\n";
        }
        File.WriteAllText("QA/LumberWagon/driving.txt",report);Open();
    }
    [MenuItem("City Forge/QA/Lumber Wagon/Preview Drive and Stop")]
    static void Preview()
    {
        ...
        root.schedule.Execute(()=>
        {
            ...
            root.schedule.Execute(()=>
            {
                var team=Team();File.WriteAllText("QA/LumberWagon/live.txt",$"selectionHit={selected} selected={world.SelectedHorseWagonName} moving={team.IsMoving} gait={team.Horse.GetComponent<ThreeDimensionalCharacterAnimator>().State} wheelTravel={team.Carriage.GetComponent<CarriageWheelController>().TotalTravelMeters}\n");
                var stop=root.Q<Button>("stop-carriage");typeof(Clickable).GetMethod("Invoke",F).Invoke(stop.clickable,new object[]{null});
            }).StartingIn(3500);
            root.schedule.Execute(()=>{Frame();File.AppendAllText("QA/LumberWagon/live.txt",$"stopped={!Team().IsMoving} speed={Team().CurrentSpeed}\n");}).StartingIn(5500);
        }).StartingIn(500);
    }
}
#endif
