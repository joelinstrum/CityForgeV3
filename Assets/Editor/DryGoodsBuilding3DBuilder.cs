...
public static class DryGoodsBuilding3DBuilder
{
 const string Root="Assets/CityForgeV3/Resources/CityForgeV3/Buildings3D/DryGoodsV05";
 const string Command="/tmp/cityforge-drygoods-command.txt";
 const string Report="/Users/joelinstrum/dev/CityForgeMCP/artifacts/buildings/dry-goods/v05-unity/runtime-report.json";
 ...
 static void Poll()
 {
  ...
  try {
   switch(cmd){case "build":Build();break;case "tests":Tests();break;case "play":EditorApplication.isPaused=false;EditorApplication.isPlaying=true;break;case "stop":EditorApplication.isPlaying=false;break;
    ...
    case "select":World()?.SelectBuilding3DForQa(0);break;case "focus":Focus();break;case "afternoon":World()?.SetTimeOfDay(TimeOfDayPreset.Afternoon);break;case "closeview":World()?.SetQaOrthographicSize(17f);break;case "deselect":World()?.DeselectBuilding3D();break;case "validate":Validate();break;case "dump":Dump();break;
   }
   File.WriteAllText("/tmp/cityforge-drygoods-result.txt",cmd+": OK");
  }catch(Exception e){Debug.LogException(e);File.WriteAllText("/tmp/cityforge-drygoods-result.txt",cmd+": "+e);}
 }
 static LotWorldController World()=>UnityEngine.Object.FindFirstObjectByType<LotWorldController>();
 [MenuItem("City Forge/3D Buildings/Create Dry Goods Package")]
 public static void Build()
 {
  ...
  var import=AssetImporter.GetAtPath(Root+"/Source/DryGoodsV05.fbx") as ModelImporter;
  ...
  var visual=UnityEngine.Object.Instantiate(AssetDatabase.LoadAssetAtPath<GameObject>(Root+"/Source/DryGoodsV05.fbx"));visual.name="DryGoodsVisual";
  try {
   ...
   var package=AssetDatabase.LoadAssetAtPath<Building3DPackage>(Root+"/DryGoodsV05.asset");if(package==null){package=ScriptableObject.CreateInstance<Building3DPackage>();AssetDatabase.CreateAsset(package,Root+"/DryGoodsV05.asset");}
   ...
   var root=new GameObject("Dry Goods");try{root.AddComponent<Building3DPackageInstance>().Configure(package);PrefabUtility.SaveAsPrefabAsset(root,Root+"/Prefabs/DryGoodsV05.prefab");}finally{UnityEngine.Object.DestroyImmediate(root);}
 ...
 class TestResults:UnityEditor.TestTools.TestRunner.Api.ICallbacks{
  ...
  public void RunFinished(UnityEditor.TestTools.TestRunner.Api.ITestResultAdaptor r){UnityEditor.TestTools.TestRunner.Api.TestRunnerApi.SaveResultToFile(r,"/Users/joelinstrum/dev/CityForgeMCP/artifacts/buildings/dry-goods/v05-unity/door-tests.xml");UnityEngine.Object.DestroyImmediate(runner);}
 }
