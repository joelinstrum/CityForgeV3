...
public static class CoveredWagonQa
{
    ...
    [MenuItem("City Forge/QA/Covered Wagon/Open Track Preview")]
    static void Open()
    {
        if(!EditorApplication.isPlaying)return;
        Fixture(LotWorldController.HorseCoveredWagonPropId);Frame();
        Directory.CreateDirectory("QA/CoveredWagon");
        ...
        File.WriteAllText("QA/CoveredWagon/rig.txt",report);
    }
    [MenuItem("City Forge/QA/Covered Wagon/Check Shared Driving")]
    static void Check()
    {
        if(!EditorApplication.isPlaying)return;
        Directory.CreateDirectory("QA/CoveredWagon");var report="";
        foreach(var id in new[]{LotWorldController.HorseCarriagePropId,LotWorldController.HorseLumberWagonPropId,LotWorldController.HorseCoveredWagonPropId})
        {
        ...
        File.WriteAllText("QA/CoveredWagon/driving.txt",report);Open();
    }
    [MenuItem("City Forge/QA/Covered Wagon/Preview Drive and Stop")]
    static void Preview()
    {
        ...
        root.schedule.Execute(()=>
        {
            ...
            root.schedule.Execute(()=>
            {
                var team=Team();File.WriteAllText("QA/CoveredWagon/live.txt",$"selectionHit={selected} selected={world.SelectedHorseWagonName} moving={team.IsMoving} gait={team.Horse.GetComponent<ThreeDimensionalCharacterAnimator>().State} secondGait={team.SecondHorse.GetComponent<ThreeDimensionalCharacterAnimator>().State} spacing={Vector3.Distance(team.Horse.position,team.SecondHorse.position)} wheelTravel={team.Carriage.GetComponent<CarriageWheelController>().TotalTravelMeters}\n");
                var stop=root.Q<Button>("stop-carriage");typeof(Clickable).GetMethod("Invoke",F).Invoke(stop.clickable,new object[]{null});
            }).StartingIn(3500);
            root.schedule.Execute(()=>{Frame();File.AppendAllText("QA/CoveredWagon/live.txt",$"stopped={!Team().IsMoving} speed={Team().CurrentSpeed}\n");}).StartingIn(5500);
        }).StartingIn(500);
    }
}
#endif
