...
namespace CityForgeV3.Tests.EditMode
{
 public class DistrictQuarryTests
 {
  ...
  [Test] public void UnpaidWorkersStopProductionAndResumeWhenFunded()
  {
   ...
   Assert.AreEqual(0,d.Treasury);Assert.AreEqual(0,d.ResourceInventory.Stone);
  ...
  [Test] public void LoadedStoneRemainsInTransitAcrossReloadAndFullCart(){var d=District();var p=d.StoneSites[0];DistrictQuarry.Build(d,p,_=>true);DistrictQuarry.Tick(d,60,_=>true);Assert.AreEqual("loading",p.Phase);Assert.AreEqual(0,d.ResourceInventory.Stone);DistrictQuarry.Tick(d,2,_=>true);d=JsonUtility.FromJson<RegionCityTile>(JsonUtility.ToJson(d));p=d.StoneSites[0];DistrictQuarry.Tick(d,0,_=>true);Assert.AreEqual(0,d.ResourceInventory.Stone);DistrictQuarry.Tick(d,2,_=>true);Assert.AreEqual(0,d.ResourceInventory.Stone);Assert.AreEqual(1,p.CartBlocks);DistrictQuarry.Tick(d,192,_=>true);Assert.AreEqual(0,d.ResourceInventory.Stone);Assert.AreEqual("full",p.Phase);DistrictQuarry.Tick(d,8,_=>true);Assert.AreEqual(4,p.CartBlocks);Assert.AreEqual(0,d.ResourceInventory.Stone);DistrictQuarry.Tick(d,64,_=>true);Assert.AreEqual(0,d.ResourceInventory.Stone);}
  ...
  [Test] public void ScriptValidationAndInventorySaturation(){Assert.Throws<ArgumentException>(()=>QuarryScript.Parse("{\"schema\":\"bad\"}"));Assert.Throws<ArgumentException>(()=>QuarryScript.Parse("{\"miningSeconds\":0}"));var d=District();var p=d.StoneSites[0];p.Script=QuarryScript.Parse("{\"miningSeconds\":1,\"loadingSeconds\":1,\"cartCapacity\":1,\"stoneTonsPerBlock\":10}");DistrictQuarry.Build(d,p,_=>true);d.ResourceInventory.Stone=int.MaxValue-1;DistrictQuarry.Tick(d,2,_=>true);Assert.AreEqual(int.MaxValue-1,d.ResourceInventory.Stone);Assert.AreEqual(10,p.CargoStoneTons);}
