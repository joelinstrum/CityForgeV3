...
namespace CityForgeV3.Tests.EditMode
{
 public class CargoLoadingBehaviorTests
 {
  ...
  [Test] public void DockedBargeCanPushOffTheShallowBankIntoTheChannel(){var go=MakeRiver(false,50,out var world);try{var route=world.FindDownstreamBoatRoute(new Vector3(6,0,0),3.25f,10);Assert.IsNotNull(route);Assert.AreEqual(new Vector3(6,0,0).x,route[0].x,.01f);Assert.AreEqual(0,route[1].x,.01f);}finally{UnityEngine.Object.DestroyImmediate(go);}}
