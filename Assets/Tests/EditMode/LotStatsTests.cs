...
lot.Stats.PlacementBonuses.Add(new LotResourceBonus{ResourceId="gold",Amount=75});
var saved=JsonUtility.FromJson<LotSaveData>(JsonUtility.ToJson(lot));
Assert.That(saved.Stats.Residents,Is.EqualTo(12));Assert.That(saved.Stats.MinimumEraId,Is.EqualTo("industrial"));Assert.That(saved.Stats.MinimumPopulation,Is.EqualTo(200));Assert.That(saved.Stats.MinimumEducationScore,Is.EqualTo(50));Assert.That(saved.Stats.RequiresRoad,Is.True);Assert.That(saved.Stats.ServiceCapacity,Is.EqualTo(100));Assert.That(saved.Stats.Benefits[0].Timing,Is.EqualTo("Per delivery"));Assert.That(saved.Stats.PlacementBonuses[0].Amount,Is.EqualTo(75));
var copy=saved.Copy();copy.Stats.ConstructionResources[0].amount=99;copy.Stats.Benefits[0].Amount=99;copy.Stats.PlacementBonuses[0].Amount=99;copy.BusinessRates.SeasonalCost=0;
Assert.That(saved.Stats.ConstructionResources[0].amount,Is.EqualTo(25));Assert.That(saved.Stats.Benefits[0].Amount,Is.EqualTo(10));Assert.That(saved.Stats.PlacementBonuses[0].Amount,Is.EqualTo(75));Assert.That(saved.BusinessRates.SeasonalCost,Is.EqualTo(200));
Assert.That(LotEconomy.ConstructionRequirements(saved)["lumber"],Is.EqualTo(25));
}
[Test] public void LegacyLotsDoNotGainRequirementsRatesOrBonuses(){var lot=JsonUtility.FromJson<LotSaveData>("{\"Name\":\"Old lot\"}");Assert.That(lot.Stats?.Residents ?? 0,Is.Zero);Assert.That(lot.Stats?.MinimumPopulation ?? 0,Is.Zero);Assert.That(lot.Stats?.MinimumEducationScore ?? 0,Is.Zero);Assert.That(lot.Stats?.PlacementBonuses?.Count ?? 0,Is.Zero);Assert.That(DistrictBusinessEconomy.Rates(lot),Is.Null);Assert.That(LotEconomy.ConstructionRequirements(lot),Is.Empty);}
}}
